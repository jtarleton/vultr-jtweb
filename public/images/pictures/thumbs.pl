...
if($#dirs>0){
    ...
    open MAINPAGE, ">index.php" or die "cannot open index.php $!";
    select MAINPAGE;
    &page_top;
	...
	$_=$dirs[$j];
	if(($j==0)&&($_=~'./')){
	    print "<a href=".'"'."./index2.php".'">'."$_</a>\n";
	}else{
	    print "<a href=".'"'."./$_/index.php".'">'."$_</a>\n";
	}
	print "<br />\n";
    }
...
sub mkthumbs{
    ...
    if(-e "pages/pictures.php"){
	unlink "pages/pictures.php";
    }

    if($top){
	open WEBPAGE, ">index2.php" or die "Cannot create file: $!";
    }else{
	open WEBPAGE, ">index.php" or die "Cannot create file: $!";
    }
    select WEBPAGE;
    &page_top;
    print "<!-- Generated by thumbnail_page.pl by jcv http://www.netpurgatory.com/ -->\n";
...
sub body{
	...
	$file=$pics[$i];
	select WEBPAGE;
	print '<a href="pages/'.($i+1).'.php">';
	print '<img src="thumbs/thum_'.$file.'" alt="'.$file.'" />';
	print "</a>\n";
	#make individual picture page
	open PAGE, ">pages/".($i+1).".php" or die "Cannot create file: $!";
	select PAGE;
	&page_top;
...
sub navigation{
	...
	if(($i==0)||($i==$#pics)){
	    ...
	    if($i==($#pics)){
		print '<a href="./'.($#pics).'.php">Previous Picture</a>'."\n";
		print '<a href="../index.php">Index</a>'."\n";
	    }
	}else{
